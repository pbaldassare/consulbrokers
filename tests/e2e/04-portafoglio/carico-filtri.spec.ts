...
test.describe('Portafoglio · Carico — filtri', () => {
  ...
  test('toggle periodo: Mese Corrente / Messe a Cassa / Tutte aggiornano URL e sottotitolo', async ({ page }) => {
    await selectCaricoPeriodo(page, 'Messe a Cassa');
    await expect(page).toHaveURL(/periodo=messe_cassa/);
    await expect(page.getByText(/Polizze messe a cassa|Tutte le polizze messe a cassa/i).first()).toBeVisible();
