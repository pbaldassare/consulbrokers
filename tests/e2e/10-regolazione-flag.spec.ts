...
test.describe.serial('Regolazione promemoria — ciclo completo', () => {
  ...
  test('4c. back/forward tra Gestione Polizze e una rotta sorella mantiene bottone+tabella allineati', async ({ page }) => {
    await page.goto(`/portafoglio/gestione?op=appendice&reg=with&q=${encodeURIComponent(POLIZZA_NUM)}`);
    await expectPageHealthy(page);
    const filterGroup = page.locator('[role="group"][aria-label="filtro-reg"]');
    await expect(filterGroup.locator('button[data-reg-filter="with"]')).toHaveClass(/bg-teal-600/);
    ...
    // Naviga a una rotta sorella del portafoglio
    await page.goto('/portafoglio/attive');
    await expectPageHealthy(page);

    // BACK → torna allo stato iniziale: bottone "In reg." attivo, riga presente
    await page.goBack();
    await page.waitForURL((url) => url.pathname === '/portafoglio/gestione', { timeout: 10_000 });
    expect(new URL(page.url()).searchParams.get('reg')).toBe('with');
    await expect(filterGroup.locator('button[data-reg-filter="with"]')).toHaveClass(/bg-teal-600/);
    ...
    // FORWARD → torna alla rotta sorella senza errori
    await page.goForward();
    await page.waitForURL((url) => url.pathname === '/portafoglio/attive', { timeout: 10_000 });
    await expectPageHealthy(page);

    // BACK di nuovo → stato integro
    await page.goBack();
    await page.waitForURL((url) => url.pathname === '/portafoglio/gestione', { timeout: 10_000 });
    expect(new URL(page.url()).searchParams.get('reg')).toBe('with');
    await expect(filterGroup.locator('button[data-reg-filter="with"]')).toHaveClass(/bg-teal-600/);
  ...
  test('4d. URL malformato (reg=invalid / reg duplicato) viene sanitizzato a filtro valido', async ({ page }) => {
    const filterGroup = page.locator('[role="group"][aria-label="filtro-reg"]');

    // Caso 1: reg=invalid → l'app deve ripristinare "Tutti" e ripulire l'URL
    await page.goto('/portafoglio/gestione?op=appendice&reg=invalid');
    await expectPageHealthy(page);
    await expect(filterGroup.locator('button[data-reg-filter="all"]')).toHaveClass(/bg-teal-600/);
    await expect.poll(() => new URL(page.url()).searchParams.get('reg')).toBeNull();

    // Caso 2: reg duplicato → vince il primo (semantica URLSearchParams) e l'URL viene normalizzato
    await page.goto('/portafoglio/gestione?op=appendice&reg=with&reg=without');
    await expectPageHealthy(page);
    await expect(filterGroup.locator('button[data-reg-filter="with"]')).toHaveClass(/bg-teal-600/);
    await expect
      .poll(() => new URL(page.url()).searchParams.getAll('reg'))
      .toEqual(['with']);

    // Caso 3: reg vuoto → normalizzato a "Tutti"
    await page.goto('/portafoglio/gestione?op=appendice&reg=');
    await expectPageHealthy(page);
    await expect(filterGroup.locator('button[data-reg-filter="all"]')).toHaveClass(/bg-teal-600/);
    await expect.poll(() => new URL(page.url()).searchParams.get('reg')).toBeNull();
  });

  test('4e. combinazione reg + q + op resta coerente attraverso navigazione e refresh', async ({ page }) => {
    const params = new URLSearchParams({ op: 'appendice', reg: 'with', q: POLIZZA_NUM });
    await page.goto(`/portafoglio/gestione?${params.toString()}`);
    await expectPageHealthy(page);

    const filterGroup = page.locator('[role="group"][aria-label="filtro-reg"]');
    await expect(filterGroup.locator('button[data-reg-filter="with"]')).toHaveClass(/bg-teal-600/);
    await expect(page.getByLabel(/N°\s*polizza\s*\/\s*ricerca libera/i)).toHaveValue(POLIZZA_NUM);

    const row = page.getByRole('row').filter({ hasText: POLIZZA_NUM });
    await expect(row).toBeVisible({ timeout: 10_000 });
    await expect(row.locator('[data-testid="reg-badge"]')).toHaveClass(/bg-yellow-100/);
    ...
    // BACK → tutti i parametri ripristinati esattamente
    await page.goBack();
    await page.waitForURL((url) => url.pathname === '/portafoglio/gestione', { timeout: 10_000 });
    const back = new URL(page.url()).searchParams;
    expect(back.get('op')).toBe('appendice');
    expect(back.get('reg')).toBe('with');
    expect(back.get('q')).toBe(POLIZZA_NUM);
    await expect(filterGroup.locator('button[data-reg-filter="with"]')).toHaveClass(/bg-teal-600/);
    await expect(page.getByRole('row').filter({ hasText: POLIZZA_NUM })).toBeVisible({ timeout: 10_000 });

    // REFRESH → stato e badge intatti
    await page.reload();
    await expectPageHealthy(page);
    expect(new URL(page.url()).searchParams.get('reg')).toBe('with');
    await expect(filterGroup.locator('button[data-reg-filter="with"]')).toHaveClass(/bg-teal-600/);
    const rowAfter = page.getByRole('row').filter({ hasText: POLIZZA_NUM });
    await expect(rowAfter).toBeVisible({ timeout: 10_000 });
    await expect(rowAfter.locator('[data-testid="reg-badge"]')).toHaveClass(/bg-yellow-100/);
  });

  test('4f. paginazione e totale risultati restano coerenti con reg=without dopo dettaglio + back', async ({ page }) => {
    await page.goto('/portafoglio/gestione?op=appendice&reg=without');
    await expectPageHealthy(page);

    // ServerPagination renderizza l'info "N–M di TOTAL" solo se totalCount > pageSize (25).
    const pagInfo = page.getByText(/^\d+–\d+\s+di\s+\d+$/);
    if (!(await pagInfo.isVisible().catch(() => false))) {
      test.info().annotations.push({
        type: 'skip-reason',
        description: 'dataset reg=without ≤ 25 righe: paginazione non renderizzata',
      });
      return;
    }

    const parseInfo = async () => {
      const txt = (await pagInfo.textContent())?.trim() ?? '';
      const m = txt.match(/^(\d+)–(\d+)\s+di\s+(\d+)$/);
      return m ? { from: +m[1], to: +m[2], total: +m[3] } : null;
    };
    const initial = await parseInfo();
    expect(initial).not.toBeNull();
    expect(initial!.from).toBe(1);
    const totalBefore = initial!.total;

    // Vai a pagina 2
    const nextBtn = page.locator('button:has(svg.lucide-chevron-right)').first();
    await nextBtn.click();
    await expect.poll(async () => (await parseInfo())?.from).toBe(26);
    expect((await parseInfo())!.total).toBe(totalBefore);

    // Apri il dettaglio della prima riga di pagina 2
    await page.getByRole('button', { name: /^Esegui$/ }).first().click();
    await page.waitForURL((url) => url.pathname.startsWith('/titoli/'), { timeout: 10_000 });

    // BACK → torna al filtro reg=without, totale invariato
    await page.goBack();
    await page.waitForURL((url) => url.pathname === '/portafoglio/gestione', { timeout: 10_000 });
    expect(new URL(page.url()).searchParams.get('reg')).toBe('without');
    await expect.poll(async () => (await parseInfo())?.total).toBe(totalBefore);

    // Passando a reg=with il totale deve cambiare e from tornare a 1 (resetPage)
    const filterGroup = page.locator('[role="group"][aria-label="filtro-reg"]');
    await filterGroup.locator('button[data-reg-filter="with"]').click();
    await expect.poll(() => new URL(page.url()).searchParams.get('reg')).toBe('with');
    const after = await parseInfo();
    if (after) {
      expect(after.total).not.toBe(totalBefore);
      expect(after.from).toBe(1);
    }
  });
