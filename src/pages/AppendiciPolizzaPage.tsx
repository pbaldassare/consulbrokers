  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Gestione appendici di polizza</p>
      </div>
        ...
        <div className="flex items-end gap-3">
          <div className="space-y-1.5 flex-1 max-w-[200px]">
            ...
            <div className="relative">
              <Input id="codice-cliente-app" value={codiceCliente} onChange={(e) => setCodiceCliente(e.target.value)} placeholder="Codice cliente" readOnly={fromDettaglio} className={fromDettaglio ? "bg-muted" : ""} />
      ...
      <fieldset className="border border-border rounded-lg p-5 space-y-4">
        ...
        <div className="flex items-end gap-4 flex-wrap">
          <div className="space-y-1.5 flex-1 min-w-[180px] max-w-[250px]">
            ...
            <div className="relative">
              <Input id="numero-polizza-app" value={numeroPolizza} onChange={(e) => setNumeroPolizza(e.target.value)} placeholder="N° polizza" readOnly={fromDettaglio} className={fromDettaglio ? "bg-muted" : ""} />
              <Search className="absolute right-2.5 top-2.5 w-4 h-4 text-muted-foreground" />
            </div>
          </div>
          <div className="space-y-1.5 w-[80px]">
            <Label htmlFor="riga-app">Riga</Label>
            <Input id="riga-app" value={riga} onChange={(e) => setRiga(e.target.value)} readOnly={fromDettaglio} className={fromDettaglio ? "bg-muted" : ""} />
          </div>
          <div className="space-y-1.5 w-[80px]">
            <Label htmlFor="appendice-app">Appendice</Label>
            <Input id="appendice-app" value={appendice} onChange={(e) => setAppendice(e.target.value)} />
          </div>
        </div>
      </fieldset>
      ...
      <fieldset className="border border-border rounded-lg p-5 space-y-4">
        ...
        <div className="flex items-center gap-2 pt-1">
          <Checkbox id="polizza-auto-app" checked={polizzaAuto} onCheckedChange={(v) => setPolizzaAuto(v === true)} />
          <Label htmlFor="polizza-auto-app" className="font-normal cursor-pointer">Polizza Auto</Label>
        </div>
      </fieldset>
