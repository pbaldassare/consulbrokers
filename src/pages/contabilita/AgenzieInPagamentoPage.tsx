...
import { Send, FileCode2, CheckCircle2, ChevronDown, ChevronRight, Trash2, Ban, Building2, Loader2, AlertCircle, Download } from "lucide-react";
...
const AgenzieInPagamentoPage = () => {
  ...
  const generaXmlMutation = useMutation({
    mutationFn: async ({ contoId, ids }: { contoId: string; ids: string[] }) => {
      const { data, error } = await supabase.functions.invoke("gestione-rimessa", {
  ...
  // PDF per agenzia (richiamato per ogni rimessa al confermo)
  const buildAndStorePdfFor = async (rimessa: Rimessa, dataPag: string) => {
    try {
      ...
      await supabase.storage.from("rimesse-pdf").upload(path, new Blob([bytes as BlobPart], { type: "application/pdf" }), { upsert: true, contentType: "application/pdf" });
      const { data: signed } = await supabase.storage.from("rimesse-pdf").createSignedUrl(path, 60 * 60 * 24 * 365);
      await supabase.from("rimessa_premi").update({ pdf_url: signed?.signedUrl || path } as any).eq("id", rimessa.id);

      // Archivio in documenti come "EC Agenzia"
      ...
      console.error("[buildAndStorePdfFor]", err);
      toast.error(`PDF rimessa ${rimessa.id.slice(0, 8)}: ${err.message}`);
    }
  };

  const confermaPagamentoMutation = useMutation({
    mutationFn: async ({ ids, data }: { ids: string[]; data: string }) => {
      // Genera PDF per ognuna
      const sel = rimesse.filter((r) => ids.includes(r.id));
      for (const r of sel) await buildAndStorePdfFor(r, data);
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="py-12 text-center text-muted-foreground">Nessuna rimessa in pagamento. Crea bozze da E/C Agenzie → "Metti in pagamento".</CardContent></Card>
      ) : (
        groups.map((g) => {
              ...
              <CardHeader className="bg-muted/40 py-3">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  <Button size="sm" variant="outline" disabled={!g.contoId || generaXmlMutation.isPending} onClick={() => handleGeneraXml(g)}>
                    <FileCode2 className="h-4 w-4 mr-1.5" /> Genera Flusso SEPA (gruppo)
                  </Button>
                  <Button
                    size="sm"
