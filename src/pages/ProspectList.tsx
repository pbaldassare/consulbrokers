...
const ProspectList = () => {
  ...
  const [form, setForm] = useState({ nome: "", cognome: "", email: "", telefono: "", fonte: "", note: "" });
  ...
  const { data: prospectResult, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (filtroSearch) q = q.or(`nome.ilike.%${filtroSearch}%,cognome.ilike.%${filtroSearch}%,email.ilike.%${filtroSearch}%`);
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      if (error) throw error;
      await logAttivita({ azione: "creazione_prospect", entita_tipo: "prospect", entita_id: data.id, dettagli_json: { nome: form.nome, cognome: form.cognome, stato: "nuovo" } });
      return data;
    },
    onSuccess: () => {
    ...
    <div className="space-y-6">
          ...
          <DialogContent className="sm:max-w-lg">
            ...
            <div className="space-y-4 pt-2">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5"><Label>Nome *</Label><Input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} placeholder="Mario" /></div>
                <div className="space-y-1.5"><Label>Cognome *</Label><Input value={form.cognome} onChange={(e) => setForm({ ...form, cognome: e.target.value })} placeholder="Rossi" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5"><Label>Email</Label><Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="email@esempio.it" /></div>
                <div className="space-y-1.5"><Label>Telefono</Label><Input value={form.telefono} onChange={(e) => setForm({ ...form, telefono: e.target.value })} placeholder="+39..." /></div>
              </div>
              ...
              <div className="space-y-1.5"><Label>Note</Label><Textarea value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} rows={3} placeholder="Note aggiuntive..." /></div>
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="outline" onClick={() => setOpen(false)}>Annulla</Button>
                <Button onClick={() => createMutation.mutate()} disabled={createMutation.isPending || !form.nome || !form.cognome}>Crea Prospect</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative">
          ...
          <Input className="pl-9 w-64" placeholder="Cerca per nome o email..." value={filtroSearch} onChange={(e) => { setFiltroSearch(e.target.value); setPage(0); }} />
        </div>
