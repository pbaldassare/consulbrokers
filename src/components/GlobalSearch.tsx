...
const NLP_MAPPINGS: { keywords: string[]; query: () => Promise<SearchResult[]> }[] = [
  ...
  },
  {
    ...
    query: async () => {
      ...
      return (data || []).map((p: any) => ({ id: p.id, titolo: `€${p.importo_provvigione?.toFixed(2)}`, sottotitolo: p.profiles ? `${p.profiles.nome} ${p.profiles.cognome}` : "—", categoria: "movimenti", link: "/cfo" }));
    },
  },
  {
