...
export function TableScrollArea({ children, className, ...props }: Props) {
  ...
  const measure = useCallback(() => {
    ...
    const width = Math.max(bottom.scrollWidth, table?.scrollWidth ?? 0);
  ...
  return (
    <div className="relative w-full" data-table-scroll-root="" {...props}>
      <div
        ref={topRef}
        data-table-h-scroll-top=""
        aria-hidden
        className={cn("overflow-x-auto overflow-y-hidden", needsHScroll ? "h-3.5" : "hidden")}
