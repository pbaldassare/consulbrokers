...
-- Funzione: genera quietanza successiva alla messa a cassa
...
BEGIN
  ...
  -- Skip poliennale (durata > 13 mesi): resta attivo da solo
  ...
  -- Skip se esiste già un successore (rinnovo pre-staged o quietanza già generata)
  ...
  -- Determina periodo in mesi dal frazionamento (testo) con fallback su rate/periodicita
  ...
  -- Base date: durata_a o data_scadenza
  ...
  -- Garanzia: stesso offset rispetto a durata_da
  ...
  -- Nuova riga nella catena numero_titolo
  SELECT COALESCE(MAX(riga), 0) + 1 INTO v_new_riga
  FROM public.titoli
  WHERE numero_titolo = NEW.numero_titolo;

  -- Inserimento nuovo titolo (quietanza successiva)
  INSERT INTO public.titoli (
    ...
    premio_netto, tasse, addizionali, provvigioni_firma,
    premio_netto_quietanza, tasse_quietanza, addizionali_quietanza, provvigioni_quietanza,
    premio_lordo,
  ...
  -- Copia righe garanzia (firma + quietanza)
...
-- Trigger: dopo trg_attiva_rinnovo_su_messa_cassa
