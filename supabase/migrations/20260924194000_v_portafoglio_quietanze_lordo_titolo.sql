-- Il lordo in Incassi/Carico/Messa a cassa deve essere titoli.premio_lordo,
-- non quietanze.premio_lordo (tabella legacy, spesso importo firma vecchio es. 1222,50).
...
SELECT
  ...
  q.data_competenza,
  q.data_scadenza,
  COALESCE(tr.premio_lordo, t.premio_lordo, q.premio_lordo) AS premio_lordo,
  COALESCE(tr.premio_netto, t.premio_netto, q.premio_netto) AS premio_netto,
  q.tasse,
  q.addizionali,
  q.ssn,
  ...
  COALESCE(tr.tipo_pagamento, t.tipo_pagamento)       AS tipo_pagamento
FROM quietanze q
...
COMMENT ON VIEW public.v_portafoglio_quietanze IS
  'Vista Incassi/portafoglio: lordo/netto da titoli, non dalla tabella legacy quietanze.';
