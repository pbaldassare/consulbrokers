-- Premio unico anticipato: split date quietanze.
-- Q1: start → end-1 giorno; Q2: end → end (decorrenza diversa).
-- Edge: se periodo = 1 giorno (start = end), entrambe restano sullo stesso giorno.
...
BEGIN
  ...
  -- Q1: start → end-1 giorno; Q2: end → end (decorrenza diversa, gestibile separatamente).
  -- Edge: periodo di 1 solo giorno → day-before non valido; Q1 e Q2 restano sullo stesso giorno.
...
'Pre-genera quietanze all''insert polizza madre. Rateo: Q1=firma, Q2+=quietanza, madre=shell. Temporanea: 1 quietanza. Premio unico anticipato: Q1 start→end-1, Q2 end→end (1 giorno: entrambe stesso giorno). Standard: frazionamento+anni_durata.';
