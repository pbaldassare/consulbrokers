...
-- Backfill quietanze già generate dal trigger: data_scadenza = garanzia_da (decorrenza)
UPDATE public.titoli
SET data_scadenza = garanzia_da
WHERE sostituisce_polizza IS NOT NULL
  AND stato = 'attivo'
  AND garanzia_da IS NOT NULL
  AND garanzia_a IS NOT NULL
  AND data_scadenza = garanzia_a
  AND data_scadenza <> garanzia_da;