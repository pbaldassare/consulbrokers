-- Annulla: aggiorna anche polizze.stato se questo titolo è la madre;
-- stacca polizza_id se era una duplica agganciata a un altro contratto;
-- pulisce FK che bloccavano il delete delle quietanze.
...
DECLARE
  ...
  v_is_madre_contratto boolean := false;
  v_result jsonb;
BEGIN
  SELECT id, numero_titolo, riga, polizza_id INTO v_titolo
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.polizze p
    WHERE p.titolo_madre_id = p_titolo_id
       OR (v_titolo.polizza_id IS NOT NULL AND p.id = v_titolo.polizza_id AND p.titolo_madre_id = p_titolo_id)
  ) INTO v_is_madre_contratto;
  ...
  DELETE FROM public.note_restituzione_dettaglio WHERE titolo_id = ANY(v_ids);
  ...
  WITH d AS (DELETE FROM public.movimenti_polizze
             WHERE titolo_id = ANY(v_ids) RETURNING 1)
  ...
  UPDATE public.titoli SET
    ...
    polizza_id = CASE WHEN v_is_madre_contratto THEN polizza_id ELSE NULL END,
    updated_at = now()
  WHERE id = p_titolo_id;

  IF v_is_madre_contratto THEN
    UPDATE public.polizze
    SET stato = 'annullata', updated_at = now()
    WHERE titolo_madre_id = p_titolo_id;
  END IF;
