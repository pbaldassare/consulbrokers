-- Clip generazione quietanze alla durata della polizza madre.
-- Non partire una rata a/dopo durata_a; garanzia_a non supera durata_a.
-- Solo CREATE OR REPLACE delle funzioni trigger: nessun UPDATE su titoli esistenti.
...
BEGIN
  ...
  -- Clip alla durata della madre: non partire una rata a/dopo durata_a
  -- (es. M16850989: 2029-06-30 → 2030-06-30).
  v_durata_fine := NEW.durata_a;

  FOR v_i IN 1..v_n_rate LOOP
    v_gar_da := (NEW.garanzia_da + ((v_i - 1) * v_months_period || ' months')::interval)::date;
    IF v_durata_fine IS NOT NULL AND v_gar_da >= v_durata_fine THEN
      EXIT;
    END IF;
    v_gar_a  := (v_gar_da + (v_months_period || ' months')::interval)::date;
    IF v_durata_fine IS NOT NULL AND v_gar_a > v_durata_fine THEN
    ...
    IF NEW.durata_da IS NOT NULL THEN
      ...
      IF v_durata_fine IS NOT NULL AND v_dur_a > v_durata_fine THEN
        v_dur_a := v_durata_fine;
      END IF;
    ELSE
...
'Pre-genera quietanze all''insert polizza madre. Temporanea/Rata unica: 1 quietanza. Rateo: Q1=firma + successive. Premio unico anticipato: 2 quietanze split date. Poliennale/standard: (12/mesi_rata)*anni_durata, clip garanzia a durata_a (no rata con garanzia_da >= durata_a).';
...
BEGIN
  ...
  -- Premio unico anticipato / Rata unica: quietanze già pre-generate; non crearne altre.
  IF EXISTS (
    SELECT 1 FROM public.titoli
    WHERE numero_titolo = NEW.numero_titolo
      AND sostituisce_polizza IS NULL
  ...
  -- Fine contratto della madre (non della rata, che ha durata slittata).
  ...
  -- Non creare uno slot che parte a/dopo durata_a (es. 2029-06-30 → 2030-06-30).
  IF v_madre_durata_a IS NOT NULL AND v_new_gar_da >= v_madre_durata_a THEN
    RETURN NEW;
  END IF;
  IF v_madre_durata_a IS NOT NULL AND v_new_gar_a > v_madre_durata_a THEN
    v_new_gar_a := v_madre_durata_a;
  END IF;
  IF v_madre_durata_a IS NOT NULL AND v_new_a > v_madre_durata_a THEN
    v_new_a := v_madre_durata_a;
  END IF;
