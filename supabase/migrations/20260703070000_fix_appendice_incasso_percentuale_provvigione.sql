-- Fix: la RPC crea_appendice_incasso inseriva in public.titoli la colonna
-- `percentuale_provvigione`, che NON esiste sulla tabella titoli (esistono solo
-- provvigioni_firma / provvigioni_quietanza). L'insert del titolo-incasso derivato
-- falliva con: column "percentuale_provvigione" of relation "titoli" does not exist.
-- La percentuale resta salvata sull'header appendice (appendici_polizza), dove la
-- colonna esiste. Qui rimuoviamo il riferimento errato dall'INSERT su titoli.
...
BEGIN
  ...
  -- Header appendice (qui percentuale_provvigione ESISTE)
  ...
  -- NB: titoli NON ha percentuale_provvigione → usare solo provvigioni_firma/quietanza.
