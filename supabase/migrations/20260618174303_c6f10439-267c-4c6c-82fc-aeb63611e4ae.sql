...
-- Trigger: alla creazione di una polizza madre, genera l'intera catena di quietanze
-- in base a frazionamento + anni_durata.
...
DECLARE
  ...
  v_new_riga int;
BEGIN
  -- Solo polizze madri (sostituisce_polizza NULL) e non regolazioni
  ...
  IF NEW.numero_titolo IS NULL THEN
    RETURN NEW;
  END IF;
  IF NEW.garanzia_da IS NULL OR NEW.garanzia_a IS NULL THEN
    RETURN NEW;
  END IF;
  ...
  -- Poliennale o frazionamento non riconosciuto → nessuna pre-generazione
  IF v_months_period = 0 THEN
    RETURN NEW;
  END IF;

  v_anni := GREATEST(1, COALESCE(NEW.anni_durata, 1));
  -- Numero totale rate (madre inclusa)
  ...
  -- Se esiste già una quietanza per questa polizza (es. legacy re-insert), esci
  IF EXISTS (
...
DROP TRIGGER IF EXISTS trg_genera_quietanze_su_insert_madre ON public.titoli;
CREATE TRIGGER trg_genera_quietanze_su_insert_madre
AFTER INSERT ON public.titoli
FOR EACH ROW
EXECUTE FUNCTION public.genera_quietanze_su_insert_madre();

COMMENT ON FUNCTION public.genera_quietanze_su_insert_madre() IS
'Pre-genera la catena di quietanze (rate 2..N) all''insert della polizza madre, in base a frazionamento + anni_durata. Skip per poliennali, regolazioni, quietanze già esistenti.';
