...
if [[ -z "${DISPLAY:-}" ]]; then
  ...
  if ! xdpyinfo -display :99 >/dev/null 2>&1; then
    if ! command -v Xvfb >/dev/null 2>&1; then
      echo "Niente DISPLAY e niente Xvfb. Installa xvfb oppure apri $URL da Cursor (porta 5175)." >&2
      exit 1
    fi
...
if [[ -z "$chrome" ]]; then
  echo "Nessun browser trovato. Apri $URL da Cursor (forward porta 5175)." >&2
  exit 1
fi
